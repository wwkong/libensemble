...
def run_covid(H, persis_info, sim_specs, libE_info):
    ...
    # try:
    #     sim_out = read_sim_output( task.workdir )
    # except Exception:
    #     sim_out = np.nan
    #     print('Warning - output is Nan')

    sim_out = np.array([0,1,2])
    libE_output = np.zeros(1, dtype=sim_specs['out'])

    libE_output['val1'] = sim_out[0]
    libE_output['val2'] = sim_out[1]
    libE_output['f'] = sim_out[2]
